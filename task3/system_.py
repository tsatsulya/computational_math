...
def build_plot(method_name, file_name):
    ...
    plt.plot([ind for ind in range(len(vector_list[0]))],
             vector_list[0], marker='o', color='cadetblue', markersize=4)
    plt.plot([ind for ind in range(len(vector_list[0]))],
             vector_list[1], marker='o', color='mediumaquamarine', markersize=4)
    plt.plot([ind for ind in range(len(norm_list))],
             norm_list, marker='o', color="palegreen", markersize=4)

    plt.legend(['x', 'y', '$|F|$'], loc='upper left', fontsize='10')

    plt.title(r'$Roots\:of\:the\:system$: ' + method_name + r' $method$')

    plt.grid(linestyle='--', linewidth=0.5)

    plt.xlabel(r'$iteration\:number$')
    plt.ylabel(r'$roots\:value$')
...
def test_newton_system():
    ...
    build_plot('Newton\'s', 'newton_method_for_systems')
    return
...
def test_iterative_method():
    ...
    build_plot('Iterative', 'iterative_method_for_systems')
    return
...
def iterative_method(F, D, x, eps):
    ...
    while abs(np.linalg.norm(F(x), ord=2)) > eps and iteration_count < 1000:
        x = D(x)
