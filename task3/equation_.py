...
def styled_plot(list_for_draw, yerr, color, shift=1, ecolor='ecolor', label_=''):
    if ecolor == "ecolor":
        ecolor = color
    x_ = np.array(range(len(list_for_draw))) + shift
    mks = 4  # markersize
    plt.plot(x_, list_for_draw, marker='o',
             color=color, markersize=mks, label=label_)
    plt.errorbar(x_, list_for_draw, yerr=yerr, fmt='o', color=color,
                 ecolor=ecolor, markersize=mks, linewidth=1, alpha=0.7)
...
styled_plot(x_list, error_list, 'tomato', 1,
            'salmon', 'Dividing a segment in half')
styled_plot(x_newton, error_newton, 'royalblue', 1, 'ecolor', 'Newton method')
styled_plot(x_iter, error_iter, 'mediumvioletred',
            len(x_list), 'ecolor', 'Iteration method')
...
styled_plot(x_newton, error_newton, 'darkslategray', len(
    x_list), 'ecolor', 'Dividing a segment in half')
styled_plot(x_iter, error_iter, 'gold', len(x_list), 'ecolor', 'Newton method')
plt.legend(loc="upper right", fontsize='7')
plt.savefig('img/root_.jpg', dpi=500)
